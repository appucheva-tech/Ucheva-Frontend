...
const Score = () => {
  ...
  const fetchSubject = async (subject) => {
    try {
      ...
      const list = res.data.studentData || [];
  ...
  const getScores = async () => {
    try {
      const res = await apiClient.get(`/classteacher/getscores/`);
  ...
  const handleClassSelectionChange = (subjectName, selectedClassesList) => {
    ...
    const relevantEntries = subjectGroup.entries.filter(
      (entry) =>
      ...
      for (const entry of entries) {
        ...
        if (Array.isArray(data)) {
          const studentsWithClass = data.map((student) => ({
            ...student,
            className:
              entry.applicableClasses && entry.applicableClasses.length > 0
  ...
  const handleSaveScores = async () => {
    try {
      ...
      const subjectId = selectedSubject?.entries?.find((e) => e.id)?.id;
      if (!subjectId) {
  ...
  return (
    <div className="CTSC-container">
      {/* HEADER */}
      <div className="CTSC-header-section">
        <h1 className="CTSC-main-title">Scores</h1>
        <p className="CTSC-subtitle">
          Select a subject and choose classes to enter scores
        </p>
      </div>

      {/* SUBJECT CARDS */}
      <div className="CTSC-subject-cards-container">
        {groupedSubject.map((group) => (
          <div
            key={group.subjectName}
            className={`CTSC-subject-card-wrapper ${
              selectedSubject?.subjectName === group.subjectName
                ? "CTSC-active"
                : ""
            }`}
          >
            <div
              className="CTSC-subject-card-header"
              onClick={() => {
                handleSubjectSelect(group);
                toggleSubjectExpansion(group.subjectName);
              }}
            >
              <div className="CTSC-subject-info">
                <div className="CTSC-subject-name-main">
                  {group.subjectName}
                </div>
                <div className="CTSC-subject-classes-count">
                  {group.entries.length} class
                  {group.entries.length > 1 ? "es" : ""} {/* ✅ FIXED */}
                </div>
              </div>

              <div className="CTSC-subject-actions">
                <span className="CTSC-expand-icon">
            ...
            {expandedSubjects[group.subjectName] && (
              <div className="CTSC-subject-card-body">
                <div className="CTSC-classes-list">
                  {group.entries.map((entry) =>
                    entry.applicableClasses?.map((className, idx) => {
                      ...
                      return (
                        <div className="CTSC-class-item" key={key}>
                          <input
                            type="checkbox"
                            checked={isChecked}
                            onChange={() =>
      ...
      <div className="CTSC-selected-subject-header">
        <div>
          <h2 className="CTSC-selected-subject-title">
            {selectedSubject ? selectedSubject.subjectName : "Select a subject"}
            {selectedSubject && selectedClasses.length > 0 && (
              <span className="CTSC-selected-classes-count">
          ...
          {selectedSubject && selectedClasses.length > 0 && (
            <div className="CTSC-selected-classes-list">
              Selected: {selectedClasses.join(", ")}
            </div>
          )}
        </div>
        <button
          className="CTSC-save-button"
      ...
      {/* TABLE */}
      <div className="CTSC-table-wrapper">
        <table className="CTSC-scores-table">
          <thead>
            <tr>
              <th>Student Name</th>
              ...
              <th>Class</th>
              <th>
                <div className="CTSC-header-cell">CA</div>
                <div className="CTSC-header-marks">(40 Marks)</div>
              </th>
              <th>
                <div className="CTSC-header-cell">Exam</div>
                <div className="CTSC-header-marks">(60 Marks)</div>
              </th>
            </tr>
          </thead>
          ...
          <tbody>
            {fetchingStudents ? (
              ...
              displayedStudents.map((student) => {
                ...
                return (
                  <tr key={id}>
                    <td className="CTSC-student-name">
                      {student.fullName ||
                        `${student.firstName || ""} ${student.lastName || ""}`.trim() ||
                        "Unknown Student"}
                    </td>
                    <td className="CTSC-admission-number">
                      {student.admissionNumber || "N/A"}
                    </td>
                    <td>
                      {student.className || student.studentClass || "N/A"}
                    </td>
                    <td>
                      <input
                        type="number"
                        className="CTSC-score-input"
                        value={scores[id]?.ca || ""}
                        onChange={(e) =>
      ...
      {/* PAGINATION */}
      {students.length > 0 && (
        <div className="CTSC-pagination-section">
          <div className="CTSC-showing-text">
            Showing {startIndex + 1} to{" "}
            {Math.min(startIndex + rowsPerPage, students.length)} of{" "}
            {students.length} students
          </div>

          <div className="CTSC-pagination-controls">
            <button
              className="CTSC-pagination-nav"
            ...
            {Array.from({ length: totalPages || 1 }, (_, i) => (
              <button
                key={i}
                className={`CTSC-pagination-number ${
                  currentPage === i + 1 ? "CTSC-active" : ""
                }`}
          ...
          <div className="CTSC-rows-per-page">
      ...
      {/* INFO */}
      <div className="CTSC-info-box">
        <span className="CTSC-info-icon">ℹ</span>
        <span className="CTSC-info-text">
          Select a subject, choose the classes you want to manage, enter scores
          for each student, and click Save Scores.
        </span>
      </div>
    </div>
  );
